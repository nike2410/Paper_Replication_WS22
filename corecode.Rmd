...
pdf("Figure 1.pdf",         # File name
    ...
    colormodel = "cmyk",    # Color model (cmyk is required for most 
                            # publications)
    paper = "A4")
par(mfrow = c(3,1))
...
for (i in (1:306)) {
  ...
  inter1[1,i] <- format(as.numeric(Result_i[1,2]), scientific=FALSE, digits = 3)
  inter1[2,i] <- format(as.numeric(Result_i[1,4]), scientific=FALSE, digits = 3)
  inter1[3,i] <- format(as.numeric(Result_i[2,2]), scientific=FALSE, digits = 3)
  inter1[4,i] <- format(as.numeric(Result_i[2,4]), scientific=FALSE, digits = 3)
  inter1[5,i] <- format(as.numeric(summary(reg)$adj.r.square))
...
for (i in (1:25)) {
  ...
  err_OI[i] <- aux_R[,i] - COSK_oest * aux_b_COSK[,i] - MKT_rest * aux_b_MKT[,i]
  err_RB[i] <- aux_R[,i] - COSK_rest * aux_b_COSK[,i] - MKT_rest * aux_b_MKT[,i]
}
E_SB <- c()
for (i in (1:25)) {
  E_SB[i] = sum((err_OI[[i]]^2) - (err_RB[[i]]^2))
}
MSE_SB = sum(E_SB)/25*12
```
...
E_SM <- c()
for (i in (1:25)) {
  E_SM[i] = sum((err_OI[[i]]^2) - (err_RB[[i]]^2))
}
MSE_SM = sum(E_SM)/25*12
```
...
E_SS <- c()
for (i in (1:25)) {
  E_SS[i] = sum((err_OI[[i]]^2) - (err_RB[[i]]^2))
}
MSE_SS = sum(E_SS)/25*12
```
...
E_SL <- c()
for (i in (1:25)) {
  E_SL[i] = sum((err_OI[[i]]^2) - (err_RB[[i]]^2))
}
MSE_SL = sum(E_SL)/25*12
```
```{r Table 3}
