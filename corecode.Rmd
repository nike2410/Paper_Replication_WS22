...
data_VIX <- select(data_VIX, -c(Open, Low, High, Close, Volume)) %>% arrange(Date) %>% filter(Date < "2012-12-31") %>% mutate(Indexed=Adj.Close/100) %>% mutate(Square=Indexed^2) %>% mutate(SqPrice=Adj.Close^2)
